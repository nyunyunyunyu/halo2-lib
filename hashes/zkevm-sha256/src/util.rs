...
pub mod decode {
    ...
    use crate::{
        ...
        halo2_proofs::plonk::Expression,
    };

    pub(crate) fn expr<F: ScalarField>(bits: &[Expression<F>]) -> Expression<F> {
...
pub mod rotate {
    ...
    pub(crate) fn expr<F: ScalarField>(bits: &[Expression<F>], count: usize) -> Vec<Expression<F>> {
...
pub mod to_bytes {
    ...
    pub(crate) fn expr<F: ScalarField>(bits: &[Expression<F>]) -> Vec<Expression<F>> {
